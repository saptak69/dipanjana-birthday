  ...
  <div className="fixed inset-0 z-0 bg-gradient-to-b from-[#0f0c29] via-[#302b63] to-[#24243e] overflow-hidden">
    ...
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
       {[...Array(2)].map((_, i) => (
          <div
             key={`shooting-${i}`}
             className="absolute h-[1px] w-[100px] md:w-[150px] bg-gradient-to-r from-transparent via-white to-transparent opacity-0 animate-shooting-star"
             style={{
                top: `${Math.random() * 50}%`,
                left: `${Math.random() * 100}%`,
...
const PhotoCard = ({ pageId, imagePath, isActive }) => {
  ...
  return (
    <div className={`
      relative w-full max-w-[280px] sm:max-w-[320px] md:max-w-[360px] aspect-[4/5]
      transition-all duration-[1200ms] ease-out
      ${isActive ? 'opacity-100 translate-y-0 rotate-0 scale-100' : 'opacity-0 translate-y-12 rotate-6 scale-90'}
    `}>
      ...
      <div className="relative h-full w-full rounded-[22px] overflow-hidden bg-gray-900 border border-white/10 shadow-2xl">
        {!imgError ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center bg-gray-900 p-6 text-center">
            ...
            <p className="text-white/40 text-[10px] md:text-sm">Add photo:<br/><span className="font-mono text-[9px] md:text-xs text-pink-300">/photos/page{pageId}.jpg</span></p>
          </div>
        )}
...
const ContentPanel = ({ data, isActive }) => (
  <div className={`
    flex flex-col items-center md:items-start text-center md:text-left w-full
  ...
  `}>
    ...
    <p className="font-cursive text-xl sm:text-2xl md:text-5xl text-transparent bg-clip-text bg-gradient-to-r from-white via-pink-100 to-purple-200 leading-snug md:leading-tight mb-6 md:mb-8 drop-shadow-lg p-1 max-w-[90%] md:max-w-none">
      "{data.text}"
    </p>
    ...
    <a 
      ...
      className="group relative flex items-center gap-3 md:gap-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl p-2 md:p-3 pr-4 md:pr-6 backdrop-blur-md transition-all hover:scale-105"
    >
       <div className="relative w-10 h-10 md:w-12 md:h-12 rounded-lg overflow-hidden bg-black flex items-center justify-center group-hover:rotate-3 transition-transform">
       ...
       <div className="flex flex-col text-left">
          <span className="text-[9px] uppercase tracking-wider text-white/50 group-hover:text-pink-300 transition-colors">Now Playing</span>
          <span className="text-xs md:text-sm font-medium text-white truncate max-w-[150px] md:max-w-[200px]">{data.song}</span>
       </div>

       <div className="flex gap-0.5 items-end h-3 md:h-4 ml-2 md:ml-4 opacity-50 group-hover:opacity-100">
    ...
    >
      ...
      <style>{`
        ...
        @keyframes shooting-star {
          ...
          100% { transform: translateX(-400px) translateY(400px) rotate(-45deg) scale(0.1); opacity: 0; }
        }
      ...
      {/* Header */}
      <div className="absolute top-0 left-0 w-full p-4 md:p-8 z-50 flex justify-between items-center pointer-events-none">
          <div>
             <h1 className="font-cursive text-2xl md:text-3xl text-pink-200 drop-shadow-md">Dipanjana</h1>
             <div className="flex items-center gap-2 mt-0.5 md:mt-1">
                <div className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse"></div>
                <span className="text-[8px] md:text-[10px] tracking-widest uppercase text-white/50 font-bold">Birthday Edition</span>
             </div>
          </div>
      </div>
